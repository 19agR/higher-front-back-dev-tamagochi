...
def main():
    ...
    while True:
        try:
            ...
            output = (f'Упс, кажется монеток на счету не хватило для оплаты!\n'
                     f'Монеток на счету: {game.tamagochi.status["coins"]:.0f}')
        except IncorrectAnswer:
            output = f'Упс, кажется нет такого варианта, попробуйте снова!'
